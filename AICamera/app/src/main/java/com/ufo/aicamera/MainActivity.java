...
public class MainActivity extends AppCompatActivity {
    ...
    private ArrayList<Float> bbox = new ArrayList<Float>();
    private ArrayList<Float> prob = new ArrayList<Float>();
    private ArrayList<Integer> index = new ArrayList<Integer>();
    private ArrayList<Recognition> results = new ArrayList<Recognition>();

    OverlayView transparentView;
    private Bbox bboxTracker;
    ...
    public native String detectionFromCaffe2(int h, int w, byte[] Y, byte[] U, byte[] V,
                                                     int rowStride, int pixelStride, boolean r_hwc);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bboxTracker = new Bbox(this);


        transparentView = (OverlayView) findViewById(R.id.transparentView);
    ...
    protected void createCameraPreview() {
        try {
            ...
            final int width = 640;//448;
            final int height = 480;//448;
            ...
            ImageReader.OnImageAvailableListener readerListener = new ImageReader.OnImageAvailableListener() {
                ...
                public void onImageAvailable(ImageReader reader) {
                    try {
                        ...
                        predictedClass = detectionFromCaffe2(h, w, Y, U, V,
                                rowStride, pixelStride, run_HWC);

                        String[] result = predictedClass.split("/");
                        for(int i = 1; i < result.length; i ++){
                            String[] infos = result[i].split("\\+");
                            String [] bbox = copyOfRange(infos, 0, 4);
                            ArrayList<Float> boundingBox = new ArrayList<Float>();
                            for (int k = 0; k < bbox.length; k++){
                                boundingBox.add(
                                        Math.max(0, Float.valueOf(bbox[k]))
                                );
                            }
                            final RectF box_ = new RectF(boundingBox.get(0), boundingBox.get(1),
                                    boundingBox.get(2), boundingBox.get(3) );
                            results.add(new Recognition( box_, Integer.valueOf(infos[5]),
                                    Float.valueOf(infos[4])));
                            /*
                            for(int j = 0; j < infos.length; j ++){
                                float sourceF = Float.valueOf(infos[j]);
                                if(j < 4){
                                    bbox.add(sourceF);
                                }
                                if(j == 4){
                                ...
                                if(j == 5){
                                    index.add((int)sourceF);
                                }
                            }
                            */
                        }

                        // TODO: pass the recognition results to bboxtracker and update the view using postInvalidate
                        transparentView.setRects(results);
                        transparentView.postInvalidate();
                        //ll = lll +""
                        //Log.d(TAG, "Couldn't load neural network.");
                        results.clear();
                        bbox.clear();
            ...
            cameraDevice.createCaptureSession(asList(surface, reader.getSurface()), new CameraCaptureSession.StateCallback() {
                @Override
